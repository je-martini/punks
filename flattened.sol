...
contract dna_punks {
    ...
    // This pseudo random function is determistic and should not be used on production
     ...
     function deterministic_pseudo_random_dna(uint256 _token_id, address _minter)
        public
        pure
        returns(uint256)
        {
            uint256 combined_params = _token_id + uint160(_minter);
            bytes memory encoded_params = abi.encodePacked(combined_params);
            bytes32 hashed_params = keccak256(encoded_params);

            return uint256(hashed_params);
        }
